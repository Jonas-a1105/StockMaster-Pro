    ...
    <?= View::component('card', [
        ...
        'content' => View::raw('
            <div class="flex items-center justify-between mb-3">
                ...
                ' . ($stockBajo > 0 ? Icons::get('error', 'w-5 h-5 text-amber-500') : Icons::get('check-circle', 'w-5 h-5 text-emerald-500')) . '
            </div>
    ...
    <div class="space-y-6">
        ...
        <?= View::component('card', [
            'content' => View::raw('
                ...
                ' : '
                    <ul class="space-y-3">
                        ' . implode('', array_map(function($idx, $prod) {
                        ...
                        }, array_keys($topProductos), $topProductos)) . '
                    </ul>
                ') . '
            ')
        ]) ?>
        ...
        <?= View::component('card', [
            'content' => View::raw('
                ...
                ' : '
                    <ul class="space-y-3">
                        ' . implode('', array_map(function($mov) {
                            $iconColor = $mov['tipo'] === 'Entrada' ? 'bg-emerald-100 dark:bg-emerald-900/30 text-emerald-600 dark:text-emerald-400' : 'bg-red-100 dark:bg-red-900/30 text-red-600 dark:text-red-400';
                            $icon = $mov['tipo'] === 'Entrada' ? '<svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 14l-7 7m0 0l-7-7m7 7V3"/></svg>' : '<svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 10l7-7m0 0l7 7m-7-7v18"/></svg>';
                            $amountColor = $mov['tipo'] === 'Entrada' ? 'text-emerald-600 dark:text-emerald-400' : 'text-red-600 dark:text-red-400';
                            $sign = $mov['tipo'] === 'Entrada' ? '+' : '-';
                            return '
                                <li class="flex items-center gap-3">
